...
var MapExample = React.createClass({
  ...
  getInitialState() {
    return {
      ...
      zoom: 16,
      annotations: [{
        coordinates: [51.505461, -0.0053744],
        'type': 'point',
        ...
        coordinates: [51.505941,-0.0055857],
        'type': 'point',
        title: 'Important!',
        ...
        coordinates: [51.506421,-0.0054106],
        'type': 'point',
        title: 'Important!',
        subtitle: 'Neat, this is a custom annotation image',
        id: 'marker3'
      }, {
        coordinates: [51.5065760,-0.0046979],
        'type': 'point',
        title: 'Important!',
        subtitle: 'Neat, this is a custom annotation image',
        id: 'marker4'
      }, {
        coordinates: [51.5063470,-0.0040485],
        'type': 'point',
        title: 'Important!',
        subtitle: 'Neat, this is a custom annotation image',
        id: 'marker5'
    ...
    return (
      <View style={styles.container}>
        <Mapbox
          ...
          ref={mapRef}
          accessToken={'pk.eyJ1Ijoic2FhZHNoZXJzIiwiYSI6ImNpcDQ4ZG1obzAwMzl2dG0ycnhlamlqankifQ.o1Ply0uocGW5aeGhdz3G0A'}
          styleURL={this.mapStyles.light}
        ...
        <View style={styles.textwrapper}>
          ...
          <Text style={styles.text} onPress={() => this.getCenterCoordinateZoomLevel(mapRef, (location)=> {
              console.log(location)
            })}>
            Get location
          </Text>
          <Text style={styles.text} onPress={() => this.getDirection(mapRef, (direction)=> {
              console.log(direction)
            })}>
            Get direction
          </Text>
          <Text style={styles.text} onPress={() => this.getBounds(mapRef, (bounds)=> {
              console.log(bounds)
            })}>
            Get bounds
          </Text>
          <Text style={styles.text} onPress={() => this.addPackForRegion(mapRef, {
              name: 'test',
              type: 'bbox',
              bounds: [0, 0, 0, 0],
              minZoomLevel: 0,
          ...
          <Text style={styles.text} onPress={() => this.getPacks(mapRef, (err, packs)=> {
              if (err) console.log(err)
              console.log(packs)
            })}>
            Get offline packs
          </Text>
          <Text style={styles.text} onPress={() => this.removePack(mapRef, 'test', (err, info)=> {
              if (err) console.log(err)
              if (info) {
